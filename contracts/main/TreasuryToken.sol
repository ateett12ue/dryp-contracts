...
contract TreasuryToken is InitializableERC20Detailed, BlackList {
    ...
    event TotalSupplyUpdated(
        uint256 totalSupply,
        uint256 admin,
        bool totalSupplyUpdated,
        address admin
    );

    event TokenRebasingEnabled(address account);
    event TokenRebasingDisabled(address account);
    ...
    address public treasuryMangerAddress = address(0);

    uint256 private _lockedPercentage;

    uint256 public _lockedSupply;
    mapping(address => uint256) private _unlockPercentageUpdates;
    mapping(address => uint256) public _isUpgraded;
    ...
    function initialize(
        ...
        uint256 _initialLockedPercentage
    ) external onlyGovernor initializer onlyInitializing {
        InitializableERC20Detailed._initialize(_nameArg, _symbolArg, 18);
        treasuryAddress = _treasuryAddress;
        adminAddress = _treasuryAdmin
        treasuryMangerAddress = msg.sender;
        lockedPercentage = _initialLockedPercentage;
    }

    modifier treasuryManager() {
        require(treasuryManager == msg.sender, "Caller is not the treasury manager");
        _;
    }
    ...
    modifier isBlacklisted() {
        require(isBlacklisted(msg.sender), "Caller is blacklisted by the admin");
        _;
    }
    ...
    */
    ...
    function _executeTransfer(
        ...
        address _to,
        uint256 _value
    ) internal isBlacklisted{
        bool isBlacklisted = isBlackListed(_to);
